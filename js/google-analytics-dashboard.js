...
// Render GA pages per session chart (removed mock data)
function renderGaPagesPerSessionChart(gaData) {
  // If the chart canvas doesn't exist, skip rendering
  if (!document.getElementById('ga-pages-per-session-chart')) return;

  // Check if we have actual data
  if (!gaData || !gaData.performance_trend || gaData.performance_trend.length === 0) {
    // Show message if no data available
    document.getElementById('ga-pages-per-session-chart').parentNode.innerHTML = `
            <p class="text-center text-gray-500 my-8">No pages per session data available</p>
        `;
    return;
  }

  const chartData = {
    labels: gaData.performance_trend.map(item => item.month),
    datasets: [{
      label: 'Pages per Session',
      data: gaData.performance_trend.map(item => item.pagesPerSession || 0),
