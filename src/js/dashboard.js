...
function updateOverviewMetrics(overview, gaData, socialData, emailData, youtubeData) {
    ...
    const totalUsers = gaData.reduce((sum, item) => sum + (item.users || 0), 0);
    ...
    const fbFollowers = socialData
        .filter(item => item.platform === 'Facebook')
        .reduce((max, item) => Math.max(max, item.followers || 0), 0);
        
    const igFollowers = socialData
    ...
    // Email metrics
    const avgOpenRate = emailData.length ? 
        emailData.reduce((sum, item) => sum + (item.open_rate || 0), 0) / emailData.length : 0;
        
    const avgClickRate = emailData.length ?
        emailData.reduce((sum, item) => sum + (item.click_rate || 0), 0) / emailData.length : 0;
    
    document.getElementById('email-open-rate').textContent = formatPercentage(avgOpenRate);
    document.getElementById('email-click-rate').textContent = formatPercentage(avgClickRate);
    
    // YouTube metrics
    const totalViews = youtubeData
        .filter(item => item.category && item.category.includes('Device'))
        .reduce((sum, item) => sum + (item.views || 0), 0);
    
    document.getElementById('youtube-views').textContent = formatNumber(totalViews);
}
